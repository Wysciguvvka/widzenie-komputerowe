...
def func(input_data: Union[str, List[np.ndarray]],
    ...
    try:
        ...
        for i in range(1, len(gray_frames)):
            next_pts, status, err = cv2.calcOpticalFlowPyrLK(gray_frames[i - 1], gray_frames[i], prev_pts, None)  # noqa
            ...
            dif = good_new - good_old
            flow_frames.append(flow)
            next_pts_data.append(next_pts)

            prev_frames.append(gray_frames[i - 1])
            next_frames.append(gray_frames[i])
        ...
        if debug:
            ...
            return np.array(prev_frames), np.array(next_frames), next_pts_data, np.array(frames)  # noqa
